document.addEventListener("DOMContentLoaded", () => {
    ...
    const blogFilterChips = document.querySelectorAll('.blog-filter-chip');
    ...
    function renderBlogCards() {
        ...
        const activeFilter = document.querySelector('.blog-filter-chip.active')?.dataset.blogFilter || 'all';
        const filtered = blogCards.filter(card => {
        ...
        renderBlogPagination(filtered.length);
    }

    function renderBlogPagination(total) {
        if (!blogPagination) return;
        const pages = Math.max(1, Math.ceil(total / blogCardsPerPage));
        ...
        for (let i = 1; i <= pages; i += 1) {
            ...
            btn.addEventListener('click', () => {
                blogPagination.querySelectorAll('button').forEach(b => b.classList.remove('active'));
                btn.classList.add('active');
                const startIndex = (i - 1) * blogCardsPerPage;
                const visibleCards = blogCards.filter(card => !card.classList.contains('is-hidden'));
                visibleCards.forEach((card, cardIndex) => {
                    card.style.display = cardIndex >= startIndex && cardIndex < startIndex + blogCardsPerPage ? 'block' : 'none';
                });
            });
        ...
        visibleCards.forEach((card, index) => {
            card.style.display = index < blogCardsPerPage ? 'block' : 'none';
        });
    }

    blogSearch?.addEventListener('input', renderBlogCards);
    blogFilterChips.forEach(chip => {
        chip.addEventListener('click', () => {
            blogFilterChips.forEach(item => item.classList.remove('active'));
    ...
    document.querySelectorAll('.blog-read-more').forEach(btn => {
        btn.addEventListener('click', () => window.openArticle(Number(btn.dataset.articleIndex)));
    });
    ...
    function renderGHUserStats(user) {
        ...
        if (starsEl) animateValue(starsEl, user.public_gists || 0, 700);
...
// ════════════════════════════════════════════════════════════
    ...
    function renderLeetCodeStats(d) {
        ...
        if (contestEl) contestEl.textContent = contestRating > 0 ? `${contestRating}` : '—';
        if (streakEl) streakEl.textContent = currentStreak > 0 ? `${currentStreak}d` : '—';
        if (topicsEl) topicsEl.textContent = totalTopics > 0 ? `${totalTopics}` : '—';

        const recentList = document.getElementById('lcRecentProblems');
        if (recentList) {
            const problems = Array.isArray(recentSolvedProblems) ? recentSolvedProblems.slice(0, 4) : [];
            recentList.innerHTML = problems.length ? problems.map(problem => `<li class="dashboard-list-item">${problem.title || problem}</li>`).join('') : '<li class="dashboard-list-item">No recent solves available.</li>';
        }
