...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!isTouchDevice && dot && outline) {
        ...
        function animateCursor() {
            ...
            outX += (mouseX - outX) * 0.45;
            outY += (mouseY - outY) * 0.45;
    ...
    if (firefliesCanvas && !prefersReducedMotion) {
        ...
        initFF();
        animateFF(); // Draws final initial state frames once 
    ...
    if (constCanvas && !prefersReducedMotion) {
        ...
        function animateConst() {
            ...
            if (cMouseX !== -1000) {
                constRAF = requestAnimationFrame(animateConst);
            } else {
                constRAF = null;
            }
        }
    ...
    // Cursor Trail Smooth Implementation
    const trailCont = document.getElementById('cursorTrailContainer');
    const prefersReducedMotionTrail = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    const isTouchDeviceTrail = window.matchMedia('(pointer: coarse)').matches;

    if (trailCont && !isTouchDeviceTrail && !prefersReducedMotionTrail) {
        let lastTrailTime = 0;
        window.addEventListener('mousemove', e => {
            const now = Date.now();
            if (now - lastTrailTime < 30) return; // Limit trail density
            lastTrailTime = now;
            ...
            p.style.backgroundColor = 'rgba(124, 77, 255, ' + (Math.random() * 0.5 + 0.2) + ')';

            trailCont.appendChild(p);
