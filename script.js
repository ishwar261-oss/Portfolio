...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        mouseMoveRAF = requestAnimationFrame(() => {
            ...
            if (back) back.style.transform = `translate(${x * 10}px,${y * 6}px)`;
            if (mid) mid.style.transform = `translate(${x * 18}px,${y * 10}px)`;
            if (front) front.style.transform = `translate(${x * 28}px,${y * 16}px)`;

            if (aboutImg) {
                aboutImg.style.transform = `translate(${x * -20}px, ${y * -20}px) rotateY(${x * 12}deg) rotateX(${y * -12}deg)`;
            }
            mouseMoveRAF = null;
        });
    });
    ...
    function animateCursor() {
        // dotLerp=0.82 → near-instant dot, outlineLerp=0.12 → smooth trailing ring
        const dotLerp = 0.82;
        const outlineLerp = 0.12;
    ...
    const scrollHandler = () => {
        if (window.scrollY > 40) {
            navEl && navEl.classList.add('scrolled');
        } else {
            navEl && navEl.classList.remove('scrolled');
        }
    };
    ...
    window.handleFormSubmit = function(e) {
        ...
        if (toast) {
            toast.innerHTML = '<i class="fas fa-check-circle"></i> Message sent! Opening mail client...';
    ...
    const skillObserver = new IntersectionObserver((entries) => {
        ...
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                // Lightweight stagger — no heavy 1s base delay
                setTimeout(() => {
    ...
    if (constCanvas) {
        ...
        function initConstellation() {
            resizeConstellation();
            constParticles = [];
            const count = Math.min(Math.floor((cWidth * cHeight) / 20000), 50);
    ...
    if (heroContent && heroSection) {
        ...
        heroSection.addEventListener('mousemove', (e) => {
            ...
            tiltRAF = requestAnimationFrame(() => {
                ...
                const tiltX = y * -3;
                const tiltY = x * 3;
                heroContent.style.transform = `perspective(1200px) rotateX(${tiltX}deg) rotateY(${tiltY}deg) translateY(${window.scrollY * 0.5}px)`;
                tiltRAF = null;
            });
        });

        heroSection.addEventListener('mouseleave', () => {
            heroContent.style.transform = `perspective(1200px) rotateX(0deg) rotateY(0deg) translateY(${window.scrollY * 0.5}px)`;
        });
    }
    ...
    if (trailContainer) {
        ...
        window.addEventListener('mousemove', (e) => {
            const now = Date.now();
            if (now - lastTrailTime < 50) return; // Throttle to ~20fps
            lastTrailTime = now;

            // Limit active particles
            while (trailContainer.children.length >= MAX_TRAIL_PARTICLES) {
    ...
    if (timelineContainer) {
        ...
        traveler.className = 'timeline-traveler';
        timelineContainer.appendChild(traveler);

        const timelineItems = timelineContainer.querySelectorAll('.timeline-item');
        const timelineBadges = timelineContainer.querySelectorAll('.timeline-badge');
        ...
        function updateTimeline() {
            ...
            const scrollStart = containerTop - windowHeight * 0.7;
            const scrollEnd = containerTop + containerHeight - windowHeight * 0.3;
            const scrollRange = scrollEnd - scrollStart;

            let progress = 0;
            if (scrollStart < 0) {
                progress = Math.min(Math.abs(scrollStart) / scrollRange, 1);
            }

            // Update timeline line height
            timelineContainer.style.setProperty('--timeline-progress', progress);
            const afterEl = timelineContainer;
            afterEl.style.cssText += `; --tl-height: ${progress * 100}%`;

            // We need to set the ::after height via a workaround
            // Since we can't directly style ::after, we use a CSS variable
            // The CSS uses transition for smooth movement
            const line = timelineContainer.querySelector('.timeline-line-animated');
            if (!line) {
                // Create a real element instead of relying on ::after
                const lineEl = document.createElement('div');
                lineEl.className = 'timeline-line-animated';
                lineEl.style.cssText = `
                    ...
                    border-radius: 2px;
                    box-shadow: 0 0 15px rgba(124, 77, 255, 0.3), 0 0 30px rgba(124, 77, 255, 0.1);
                    z-index: 0;
                    pointer-events: none;
                `;
            ...
            const animatedLine = timelineContainer.querySelector('.timeline-line-animated');
            if (animatedLine) {
                animatedLine.style.height = `${progress * 100}%`;
            }

            // Move traveler dot
            const travelerPos = progress * containerHeight;
            traveler.style.top = `${travelerPos}px`;

            // Show/hide traveler
            traveler.style.opacity = progress > 0 && progress < 1 ? '1' : '0';
        }

        // IntersectionObserver for timeline items and badges
        const timelineObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    entry.target.classList.add('visible');
                    // Also reveal the badge inside this item
