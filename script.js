document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderOtherRepos(repos) {
        const container = document.getElementById('github-repos-container');
        if (!container) return;
        
        // Exclude the main 4 projects featured above, plus config repos if any
        const mainProjects = ['railease', 'idzide', 'mohini', 'password-security-analyzer', 'ishwar261-oss.github.io'];
        const otherRepos = repos.filter(r => !mainProjects.some(m => r.name.toLowerCase().includes(m)) && !r.fork);
        
        if (otherRepos.length === 0) {
            container.innerHTML = '<p style="color:var(--text-muted); text-align:center; grid-column:1/-1;">No other public repositories found.</p>';
            return;
        }
